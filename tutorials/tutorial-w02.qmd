...
1.  Download the file with the data by right-clicking on the following link and download the file: [shallow.csv](../data/shallow.csv). (Note that tutorial files are also linked in the [Syllabus](../content.qmd))
...
You can easily import other file types as long as they are tabular or rectangular (i.e. spreadsheet-like), like Excel files.
...
This is the best format to save data in because it is basically a plain text file, it's quick to parse, and can be opened and edited with any software (plus, it's not a proprietary format).
...
-   Use `read_tsv()` from the readr package (installed with the tidyverse) to read tab separated values files.
