...
The model summary reports information on the beta coefficients.
:::
:::
...
as_draws_df(resp_bm) %>%
  ...
  summarise(
    TM_p_q60_lo = round(quantile2(TM_d, probs = 0.025), 2),
    TM_p_q50_hi = round(quantile2(TM_d, probs = 0.975), 2),
  )
```

So at 60% probability, there is an increase of 33 to 50 percent points in the probability of a "takete" response when the word is a "takete" word relative to when it is a "maluma" word.

We could then report the following:

> We can be 60% confident that the probability of a "takete" response is between 28% and 33% ($\beta$ = -0.81, SD = 0.15) with the "maluma" stimuli, and it is between 70% and 75% with the "takete" stimuli. At 60% confidence, the difference in probability between the "takete" and "maluma" stimuli is between 33 and 50 percent points ($\beta$ = 1.78, SD = 0.21).
