...
author: "<strike>Dr Stefano Coretta</strike> Elizabeth Pankratz"
...
ggplot() +
  ...
  geom_text(aes(x = 2.5, y = -1), label = "y = log(x)", size = 8, colour = '#8856a7')
```
...
- The mean log-odds of getting a correct response is 1.02 (corresponds to a **probability of `r round(plogis(1.02), 2)`.**)
- 95% probability that the true mean is between 0.71 and 1.36 (corresponds to a **probability between `r round(plogis(0.71), 2)` and `r round(plogis(1.36), 2)`**).
]
...
quant_1 <- ggplot() +
  ...
  annotate("label", x = qnorm(c(0.25, 0.5, 0.75)), y = 0.5, label = c("1st quartile", "2nd quartile", "3rd quartile")) +
  ylim(0, 0.6) +
  labs(
    x = element_blank(), y = element_blank(),
    title = "The Standard Gaussian distribution",
