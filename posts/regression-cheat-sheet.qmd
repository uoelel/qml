...
This is reposted from <https://stefanocoretta.github.io/posts/2021-08-21-regression-models-a-cheat-sheet/>, with minor edits.
...
Same goes with "linear models", "logistic regression", "generalised regression models", "mixed-effects regression" and so on.
...
* The variable is a *scale*: **ordinal** regression model.

    * Likert scales and ratings, language attitude questionnaires.

    * Ordinal regression models, a.k.a. ordinal logistic regression, can be fitted with the [brms](https://paul-buerkner.github.io/brms/) package using `brm(family = cumulative)`.
    
* The variable is categorical with more than two levels, but it is not ordinal: categorical (multinomial) regression model.

    * Categorical models can be fitted with the [brms](https://paul-buerkner.github.io/brms/) package using `brm(family = categorical)`.
...
Here is where so-called "random effects" or "varying effects/terms" come in.
...
If you don't include any random effect/varying terms, your model will expect that each observation is independent and hence it will underestimate variance and return unreliable results.
...
If you have other predictors in the model, you should also add them as (random) slopes in the random effects/varying terms.
For example: `(question | participant) + (question | word)` (where `question` = statement vs question).

A quick terminological note: models that include random effects/varying terms are variably called:
