...
For example, we might want to estimate the number of direct vs indirect objects based on the counts found in a corpus of English.

$$
\begin{align}
\text{N}_i & \sim Poisson(\lambda_i) \\
\ln(\lambda_i) & = \alpha_{\text{O}_i}
\end{align}
$$

- $\text{N}_i$ is the number of objects.
- $Poisson(\lambda_i)$ tells us that the number of objects follows a Poisson distribution.
- $\ln()$ is simply the natural logarithm.
- $\ln(\lambda_i)$ is equal to $\alpha$, which differs depending to the type of the object ($\alpha_{\text{O}_i}$), where $\text{O}_i$ is either "direct" or "indirect".
